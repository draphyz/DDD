...
namespace DDD.HealthcareDelivery.Infrastructure.Prescriptions
{
    ...
    public class BelgianCreatePharmaceuticalPrescriptionsValidatorTests
    {
        ...
        [Theory]
        [InlineData("042652829", null)]
        [InlineData("042652829", "")]
        [InlineData("042652829", "  ")]
        [InlineData(null, "04.265.28.29")]
        [InlineData("", "04.265.28.29")]
        [InlineData("  ", "04.265.28.29")]
        [InlineData("042652829", "02/3254879")]
        public void Validate_WhenElectronicAndBothPrescriberTelephoneNumbersNotEmpty_ReturnsNoSpecificFailure(string primaryTelephoneNumber, string secondaryTelephoneNumber)
        {
            // Arrange
            ...
            {
                IsElectronic = true,
                PrescriberPrimaryTelephoneNumber = primaryTelephoneNumber,
                PrescriberSecondaryTelephoneNumber = secondaryTelephoneNumber
            };
            // Act
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "PrescriberTelephoneNumbersEmpty");
        }
        ...
        [InlineData("")]
        [InlineData("  ")]
        public void Validate_WhenElectronicAndInCenterAndPrescriberSocialSecurityNumberEmpty_ReturnsExpectedFailure(string prescriberSocialSecurityNumber)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "PrescriberSocialSecurityNumberEmpty" && f.Severity == Severity.Error);
        }

        [Theory]
        [InlineData("aa")]
        [InlineData("11111")]
        public void Validate_WhenElectronicAndInCenterAndPrescriberSocialSecurityNumberNotEmpty_ReturnsNoSpecificFailure(string prescriberSocialSecurityNumber)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "PrescriberSocialSecurityNumberEmpty");
        }
        ...
        [InlineData("")]
        [InlineData("  ")]
        public void Validate_WhenElectronicAndInHospitalAndHealthFacilityCodeEmpty_ReturnsExpectedFailure(string healthFacilityCode)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            {
                IsElectronic = true,
                HealthFacilityType = HealthFacilityType.Hospital,
                HealthFacilityCode = healthFacilityCode
            };
            // Act
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "HealthFacilityCodeEmpty" && f.Severity == Severity.Error);
        }

        [Theory]
        [InlineData("aa")]
        [InlineData("1111111")]
        public void Validate_WhenElectronicAndInHospitalAndHealthFacilityCodeNotEmpty_ReturnsNoSpecificFailure(string healthFacilityCode)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "HealthFacilityCodeEmpty");
        }
        ...
        [InlineData("")]
        [InlineData("  ")]
        public void Validate_WhenElectronicAndInHospitalAndHealthFacilityLicenseNumberEmpty_ReturnsExpectedFailure(string healthFacilityLicenseNumber)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "HealthFacilityLicenseNumberEmpty" && f.Severity == Severity.Error);
        }

        [Theory]
        [InlineData("aa")]
        [InlineData("1111111")]
        public void Validate_WhenElectronicAndInHospitalAndHealthFacilityLicenseNumberNotEmpty_ReturnsNoSpecificFailure(string healthFacilityLicenseNumber)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "HealthFacilityLicenseNumberEmpty");
        }
        ...
        [InlineData(null, "")]
        [InlineData(" ", null)]
        public void Validate_WhenElectronicAndMedicationPosologyAndDurationEmpty_ReturnsExpectedFailure(string posology, string duration)
        {
            // Arrange
            var validator = CreateValidator();
            var command = CommandWithMedications
            (
                new PrescribedMedicationDescriptor { Posology = posology, Duration = duration }
            );
            command.IsElectronic = true;
            // Act
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "MedicationPosologyAndDurationEmpty" && f.Severity == Severity.Error);
        }
        ...
        [InlineData("", "1 mois")]
        [InlineData("  ", "1 mois")]
        public void Validate_WhenElectronicAndMedicationPosologyOrDurationNotEmpty_ReturnsNoSpecificFailure(string posology, string duration)
        {
            // Arrange
            var validator = CreateValidator();
            var command = CommandWithMedications
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "MedicationPosologyAndDurationEmpty");
        }

        [Fact]
        public void Validate_WhenElectronicAndPatientBirthdateEmpty_ReturnsExpectedFailure()
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "PatientBirthdateEmpty" && f.Severity == Severity.Error);
        }

        [Fact]
        public void Validate_WhenElectronicAndPatientBirthdateNotEmpty_ReturnsNoSpecificFailure()
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            {
                IsElectronic = true,
                PatientBirthdate = new DateTime(1980, 2, 12)
            };
            // Act
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "PatientBirthdateEmpty");
        }
        ...
        [InlineData("")]
        [InlineData("  ")]
        public void Validate_WhenElectronicAndPatientSocialSecurityNumberEmpty_ReturnsExpectedFailure(string patientSocialSecurityNumber)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "PatientSocialSecurityNumberEmpty" && f.Severity == Severity.Error);
        }

        [Theory]
        [InlineData("aa")]
        [InlineData("11111")]
        public void Validate_WhenElectronicAndPatientSocialSecurityNumberNotEmpty_ReturnsNoSpecificFailure(string patientSocialSecurityNumber)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "PatientSocialSecurityNumberEmpty");
        }
        ...
        [InlineData("")]
        [InlineData("  ")]
        public void Validate_WhenElectronicAndPrescriberCityEmpty_ReturnsExpectedFailure(string prescriberCity)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "PrescriberCityEmpty" && f.Severity == Severity.Error);
        }

        [Theory]
        [InlineData("BOELHE GEER")]
        [InlineData("Montegnée")]
        public void Validate_WhenElectronicAndPrescriberCityNotEmpty_ReturnsNoSpecificFailure(string prescriberCity)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "PrescriberCityEmpty");
        }
        ...
        [InlineData("")]
        [InlineData("  ")]
        public void Validate_WhenElectronicAndPrescriberStreetEmpty_ReturnsExpectedFailure(string prescriberStreet)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "PrescriberStreetEmpty" && f.Severity == Severity.Error);
        }

        [Theory]
        [InlineData("R Villereau, 42")]
        [InlineData("Voie des Vaux, 319")]
        public void Validate_WhenElectronicAndPrescriberStreetNotEmpty_ReturnsNoSpecificFailure(string prescriberStreet)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().NotContain(f => f.ErrorCode == "PrescriberStreetEmpty");
        }
        ...
        [InlineData("", "  ")]
        [InlineData("  ", "  ")]
        public void Validate_WhenElectronicAndPrescriberTelephoneNumbersEmpty_ReturnsExpectedFailure(string primaryTelephoneNumber, string secondaryTelephoneNumber)
        {
            // Arrange
            var validator = CreateValidator();
            var command = new CreatePharmaceuticalPrescriptions
            ...
            var results = validator.Validate(command);
            // Assert
            results.Errors.Should().ContainSingle(f => f.ErrorCode == "PrescriberTelephoneNumbersEmpty" && f.Severity == Severity.Error);
        }
