...
namespace DDD.Core.Infrastructure.Testing
{
        ...
        where TContext : BoundedContext, new()
    {
        ...
        public IDbConnectionProvider<TContext> CreateConnectionProvider(bool pooling = true)
        {
            ...
            var settings = new DbConnectionSettings<TContext>(connectionSettings.ProviderName, connectionString);
            return new LazyDbConnectionProvider<TContext>(new TContext(), settings);
        }
